...
def validity_check(x, y1, y2, points, corridor, curve, lr, c1, c2, c3, c4, c5):
    points = np.array(points)
    def is_inCircle(p, c1, c2, c3, c4, c5, curve):
        inside = False
        for i in range(len(p)):
            ...
            if (x - c4[0])**2 + (y - c4[1])**2 <= c4[2]**2:
                print('Circ4', curve)
                inside = True
            if (x - c5[0])**2 + (y - c5[1])**2 <= c5[2]**2:
                print('Circ5', curve)
                inside = True

        return inside
    in_circle = is_inCircle(points, c1, c2, c3, c4, c5, curve)
    if lr == 1:
...
if __name__ == "__main__":
    ...
    hkr1 = [304.8, 60.96, 30.48]
    hkr2 = [381, 121.92, 30.48]
    hkr3 = [426.72, 182.88, 15.24]
    hkr4 = [365.76, 229, 22.9]
    hkr5 = [219.456, 243.84, 45.72]

    xc1, yc1, m1 = make_circle(304.8, 60.96, 30.48)
    xc2, yc2, m2 = make_circle(381, 121.92, 30.48)
    xc3, yc3, m3 = make_circle(426.72, 182.88, 15.24)
    xc4, yc4, m4 = make_circle(365.76, 229, 22.9)
    xc5, yc5, m5 = make_circle(219.456, 243.84, 45.72)
    ...
    while not valid1 or not valid2:
        ...
        valid1 = validity_check(koz_x, koz_bot, koz_top, wpts_all1, corridor, 'Curve 1', lr, hkr1, hkr2, hkr3, hkr4, hkr5)
        valid2 = validity_check(koz_x, koz_bot, koz_top, wpts_all2, corridor, 'Curve 2', lr, hkr1, hkr2, hkr3, hkr4, hkr5)
        ...
        if optim_sol2[1] != m_p12*optim_sol2[0]+b:
                valid2 = False
    ...
    c1 = plt.Circle((hkr1[0],hkr1[1]), hkr1[2], color = 'r', linestyle = 'dashdot', fill = False, label = 'Obstacle')
    c2 = plt.Circle((hkr2[0],hkr2[1]), hkr2[2], color = 'r', linestyle = 'dashdot', fill = False)
    c3 = plt.Circle((hkr3[0],hkr3[1]), hkr3[2], color = 'r', linestyle = 'dashdot', fill = False)
    c4 = plt.Circle((hkr4[0],hkr4[1]), hkr4[2], color = 'r', linestyle = 'dashdot', fill = False)
    c5 = plt.Circle((hkr5[0],hkr5[1]), hkr5[2], color = 'r', linestyle = 'dashdot', fill = False)
    # ax.plot(xc1, yc1, linestyle = 'dashed', color = 'red')
    # ax.plot(xc2, yc2, linestyle = 'dashed', color = 'red')
    # ax.plot(xc3, yc3, linestyle = 'dashed', color = 'red')
    # ax.plot(xc4, yc4, linestyle = 'dashed', color = 'red')
    # ax.plot(xc5, yc5, linestyle = 'dashed', color = 'red')
    ax.add_patch(c1)
    ax.add_patch(c2)
    ax.add_patch(c3)
    ax.add_patch(c4)
    ...
    ax.text(optim_sol2[0]+0.25,optim_sol2[1],  r'$\bf{p_{12}}$')
    ax.text(nodes2[0][2]+0.25,nodes2[1][2]+20,  r'$\bf{p_{22}}$')
    ax.text(nodes1[0][2]-85,nodes1[1][2]-25,  r'Curve 1', fontsize = 6)
    ax.text(nodes1[0][2]-100,nodes1[1][2]+50,  r'Curve 2', fontsize = 6)
