...
bs.traf.cre(f'AC20', 'M250', kcmh[0], kcmh[1], 270, 0, 0)
print(bs.traf.id[20])
id = bs.traf.id[20]
...
if scen == 'Bez':
    for i in range(n_steps):
        ...
        for j in range(len(bs.traf.id)):
            ...
            # print(dist, center)
            # if min_centx <= pos[0] <= max_centx and min_freezey <= pos[1] <= max_freezey and sorter[j] ==  0:
            if bs.traf.id[j] == id and ascend == 0:
                bs.stack.stack(f'VS AC{j} 5.72008')
                bs.stack.stack(f'SPD AC{j} 54.84')
                bs.stack.stack(f'ALT AC{j} 1524') 
                ascend = 1
            # if bs.traf.id[j] == id and ascend == 2:
            #     print(run_id[j])
            if dist <= center and sorter[j] == 0:
                # print(f'{bs.traf.id[j]} crossed Optimization Horizon at timestep {i}!')
                # print(f'Postion is {bs.traf.lat[j], bs.traf.lon[j]}, horizon bounds are\nLON: {min_centlon},{max_centlon} LAT: {min_freezelat},{max_freezelat}')
            ...
            if closed[j] == c-1:
                ...
                if j == ntraf-1:
                    if gate[j] == 'I':
                        t_tran[j] = [t_tran[j][0], 99999]
                    if gate[j] == 'II':
                        t_tran[j] = [99999,t_tran[j][1]]
                    if gate[j] == 'III':
                        t_tran[j] = [99999,t_tran[j][1]]
                    if gate[j] == 'IV':
                        t_tran[j] = [t_tran[j][0], 99999]
