...
def solve_optimEntry(guess, max_bank,  min_bank, nodes, velocity, pos):
    def path_cost(ba, nodes, velocity):
        diff = find_diff_entry(ba, nodes, velocity, pos)
    ...
    t_val = find_diff_entry(val.x, nodes, velocity, pos)[1]
    return val.x, t_val
...
def find_diff_entry(ba, nodes, velocity, pos):
    pi = np.pi
    t_guess = 0.5
    mindiff = 50
...
def toCallOutside(velocity, turn_rate, target_toa1, target_toa2, uav_head, nodes1, nodes2, koz_x, koz_bot, koz_top, lr, latlon, corridor):
    # velocity  = 188 #ft/s
...
# UTM CONVERSIONS
    ...
    while not valid1 or not valid2:
        ...
        x_slope = [i for i in np.linspace(optim_sol1[0]-10, 245, 50)]
...
def EntryExitOutside(nodes1, nodes2, pos, velocity):
    vel_knots = 111.6
    print(nodes1)
    ...
    ba, t_entry = solve_optimEntry(np.deg2rad(28), np.deg2rad(30), np.deg2rad(20), nodes1, vel_knots, pos)
