...
def solve_optim2(P0, P2, target_toa,  guess, target_heading, velocity, turn_radius, lr, line):#turn_radius,
    ...
    if lr == 1:
        cons = (
                ...
                # {'type': 'ineq', 'fun': lambda x: x[0] - 1000},
                {'type': 'ineq', 'fun': lambda x: x[1] - P0[1]},
                {'type': 'eq', 'fun': lambda x: (P2[1]+0.0001) - x[1]},
...
def solve_optimEntry(guess, max_bank,  min_bank, nodes, velocity, latlon):
    def path_cost(ba, nodes, velocity):
        diff = find_diff_entry(ba, nodes, velocity, latlon)
    ...
    t_val = find_diff_entry(val.x, nodes, velocity, latlon)[1]
    return val.x, t_val
...
def find_diff_entry(ba, nodes, velocity, latlon):
    pi = np.pi
    t_guess = 0.75
    mindiff = 50
    ...
    plt.plot(Bx(tt), By(tt))
    plt.scatter(latlon[1], latlon[0])
    plt.show()
    dist = __WSG84_To_Meters_Single(latlon, [By(tt)[0], Bx(tt)[1]], Proj("EPSG:32667"))
    print('DISTDISTDIAT)', [By(tt)[0], Bx(tt)[1]], dist)
    tr = velocity**2 / (11.26*math.tan(ba[0]))
    tr_l = Meters_To_WSG84([tr, 0], latlon)
    h = tr_l[1]# + latlon[1]
    k = latlon[0]
    circle_eq = lambda t: (Bx(t)-h)**2+(By(t)-k)**2 - tr_l[1]**2
    ...
    for i in S:
        ...
        if i>=0 and i <= 1:
            ...
            x_l = [i for i in np.linspace(latlon[0], Bx(i), 200)] #Space between nominal path and bez
            print('X_L',x_l)
            y = [k+np.sqrt(tr_l[1]**2 - (x-h)**2) for x in x_l] #arc created by turn
...
def find_diff_exit(guess, nodes, velocity, latlon):
    ba = guess[0]
    t_guess = guess[1]
    ...
    circle_eq = lambda y: (latlon[0]-h(t_guess))**2+(y-k(t_guess))**2 - tr**2
    y_guess = 2000
    S = fsolve(circle_eq, y_guess) #gives y intersection
    t_final = .5

    y_l = [i for i in np.linspace(latlon, S, 200)]
    x = [latlon[0] for i in y_l]
...
def entryPath(velocity, ba, intersect, latlon):
    ...
    x_entry = [i for i in np.linspace(latlon[1], intersect[0], 200)]
...
def toCallOutside(velocity, turn_rate, target_toa1, target_toa2, uav_head, nodes1, nodes2, koz_x, koz_bot, koz_top, lr, latlon):
    velocity  = 188 #ft/s
    ...
    paths = [optimal_bez1, optimal_bez1]
    wpts, x_wpts, y_wpts = paths_to_wp(paths, 5)


    return wpts, x_wpts, y_wpts, optimal_bez1, optimal_bez2, nodes1, nodes2 

def EntryExitOutside(nodes1, nodes2, latlon):
    vel_knots = 111.6
    print(nodes1)
    ba, t_entry = solve_optimEntry(np.deg2rad(25), np.deg2rad(30), np.deg2rad(20), nodes1, vel_knots, latlon)
    
    x_int_en, y_int_en = find_bez_xy([nodes1[0][0],nodes1[1][0]],
                                [nodes1[0][1],nodes1[1][1]],
                                [nodes1[0][2],nodes1[1][2]], t_entry)
    x_entry, y_entry, central_angle, entryLength, entryTOA, h_c, k_c = entryPath(188, np.rad2deg(ba), [x_int_en, y_int_en], latlon)
    ...
    baEx, exit_t = solve_optimExit([np.deg2rad(27.5), t_start2], np.deg2rad(30), np.deg2rad(20), t_start2, nodes2, 188)
    ...
    pb1_length = optim1_length - path_length(P0=[nodes1[0][0],nodes1[1][0]], P1=[nodes1[0][1],nodes1[1][1][1]],P2=[nodes1[0][2], nodes1[1][2]], t=t_entry)
    pb2_length = path_length(P0=[nodes2[0][0],nodes2[1][0]], P1=[nodes2[0][1],nodes2[1][1]], P2=[nodes2[0][2],nodes2[1][2]], t=t_start2)
    ...
    plt.show()
    return [x_entry, y_entry, ba], [x_exit, y_exit, baEx]
...
if __name__ == "__main__":
    ...
    if lr == -1:
        ...
        nodes2 = [np.array([1475, 1525, 750]).flatten(),np.array([h/2, h, h]).flatten()]
