...
def to_sql(spacing, exptype, scen):
    ...
    if 'Bez' in (exptype or scen):
        ...
        for key, value in BezierData.items():
            records.append({
                # 'ID': key,
                'p0x': value['p0x'],
                'p0y': value['p0y'],
        ...
        records = []
        for key, value in DubinsData.items():
            records.append({
                # 'ID': key,
                'path_type': value['path_type'],
                'bez_intx': value['bez_intx'],
                'bez_inty': value['bez_inty'],
                'nom_intx': value['nom_intx'],
                'nom_inty': value['nom_inty'],
                'bez_t': value['bez_t'],
                'intersect_heading': value['intersect_heading'],
                'h': value['h'],
                'k': value['k'],
                'bank_angle': value['bank_angle'],
                'tr': value['tr'],
                'timeStamp': value['timeStamp'],
                'ACID': value['ACID'],
                'ExpNum': value['ExpNum'],
